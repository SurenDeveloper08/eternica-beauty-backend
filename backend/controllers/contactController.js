...
exports.Contact = catchAsyncError(async (req, res, next) => {
  ...
  try {
    ...
    const newContact = new Contact({ firstName, lastName, email, mobile, subject, message });
    await newContact.save();

    res.json({ success: true, message: 'Message sent and stored successfully!' });

    await sendContactEmail(process.env.ADMIN_EMAIL1, firstName, lastName, email, mobile, subject, message);
